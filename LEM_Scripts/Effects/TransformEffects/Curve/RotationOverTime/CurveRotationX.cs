...
namespace LEM_Effects
{
    ...
    [AddComponentMenu("")] public class  CurveRotationX : SingleCurveBasedUpdateEffect<CurveRotationX>
#if UNITY_EDITOR
...
#endif
    {
        ...
        {
            CurveRotationX t = go.AddComponent<CurveRotationX>();
        ...
        {
            m_Timer += delta;
            m_TargetTransform.localRotation = Quaternion.Euler(m_Graph.Evaluate(m_Timer), 0, 0);
        }

        private void RotateInWorld(float delta)
        {
            m_Timer += delta;
            m_TargetTransform.rotation = Quaternion.Euler(m_Graph.Evaluate(m_Timer), 0, 0);
        }
